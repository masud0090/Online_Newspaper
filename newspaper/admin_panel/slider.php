              ...
              <div style="padding:20px; margin-left:70px; margin-top:40px; border:1px solid black;">
                <div class="panel panel-primary">
                  ...
                  <div class="panel-body">
                    <form action="slider_connect.php" method="post" onSubmit="return checkduplicatecategories(this)">

                      <div class="form-group">
                        <label for="exampleFormControlSelect2"><B>ENTER SLIDER NEWS</B></label>
                        <select class="form-control" id="exampleFormControlSelect2" name="news_id" required>
                          <?php
                          $query = "select * from news_details where status='published'";
                          $res = mysqli_query($con, $query);
                          ?>
                          <?php while ($rows = mysqli_fetch_array($res)) {
                          ?>
                            <option value="<?php echo $rows['news_id']; ?>"> <?php echo $rows['news_headline']; ?></option>
